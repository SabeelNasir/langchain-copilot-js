...
/**
 * Generating fictional employee data using LLM
 * @returns
 */
const generateSyntheticData = async (): Promise<Employee[]> => {
  const prompt = `
  You are a helpful assistant that generates employee data. 
  Generate 10 fictional employee records. 
  Each record should include the following fields: employee_id, first_name, last_name, date_of_birth, address, contact_details, job_details, work_location, reporting_manager, skills, performance_reviews, benefits, emergency_contact, notes. 
...
async function seedDatabase(): Promise<void> {
  try {
    ...
    await client.db("hr_database").command({ ping: 1 });
    ...
    const recordsWithSummary = await Promise.all(
      syntheticData.map(async (data) => ({
        pageContent: await createEmployeeSummary(data),
        metadata: { ...data },
      }))
    );

    console.log("recordsWithSummary", recordsWithSummary);

    // Now create text-embeddings and store in mongodb Vector Database
